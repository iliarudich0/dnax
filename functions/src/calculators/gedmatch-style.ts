 ...
 */
export const COMPREHENSIVE_AIMS: { [rsid: string]: any } = {
 ...
 */
export const POPULATION_FREQUENCIES: {
  [rsid: string]: {
    [population: string]: { [allele: string]: number };
  };
} = {
  "rs1426654": {
 ...
 */
export function calculateEurogenes(snps: SNP[]): CalculatorResult {
  const populations = {
    "North_European": 0,
    "South_European": 0,
    "West_Asian": 0,
  ...
  let markersUsed = 0;
  const totalMarkers = Object.keys(COMPREHENSIVE_AIMS).length;

  // Basic ancestry inference from available SNPs
  for (const snp of snps) {
    if (POPULATION_FREQUENCIES[snp.rsid]) {
      markersUsed++;
 ...
 */
export function calculateAncestry(snps: SNP[]): CalculatorResult {
  ...
  let markersUsed = 0;
  const scores: { [pop: string]: number } = {};
  ...
  for (const [rsid, freqs] of Object.entries(POPULATION_FREQUENCIES)) {
